...
namespace NBoardLocalGameServer.Reversi
{
    ...
    internal static class Constants
    {
        ...
        public const int NumSquares = BoardSize * BoardSize;
        public const int MaxScore = NumSquares;
        public static ReadOnlySpan<ulong> CoordToBit => _CoordToBit;

        static readonly ulong[] _CoordToBit;

        static Constants()
        {
            _CoordToBit = new ulong[NumSquares];
            for (var i = 0; i < _CoordToBit.Length; i++)
                _CoordToBit[i] = 1UL << i;
        }
    }

    internal enum DiscColor : byte
    {
        Black = 0,
        White = 1,
        Null = 2
    ...
    internal struct Move(DiscColor color = DiscColor.Null, BoardCoordinate coord = BoardCoordinate.Null, ulong flip = 0)
    {
        public DiscColor Color { get; set; } = color;
    ...
    internal static class ReversiTypes
    {
        ...
        public static BoardCoordinate ParseCoordinate(ReadOnlySpan<char> str)
        {
            ...
            return (BoardCoordinate)((lstr[0] - 'a') + (lstr[1] - '1') * BoardSize);
        }
    }
}