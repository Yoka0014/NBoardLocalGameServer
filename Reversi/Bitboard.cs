...
namespace NBoardLocalGameServer.Reversi
{
    ...
    /// </summary>
    internal struct Bitboard(ulong player, ulong opponent)
    {
        public ulong Player { get; set; } = player; public ulong Opponent { get; set; } = opponent;
        public readonly ulong Discs => Player | Opponent;
        public readonly ulong Empties => ~Discs;
        public readonly int PlayerDiscCount => BitOperations.PopCount(Player);
        public readonly int OpponentDiscCount => BitOperations.PopCount(Opponent);
        public readonly int DiscCount => BitOperations.PopCount(Discs);
        public readonly int EmptyCount => BitOperations.PopCount(Empties);
        ...
        public override readonly bool Equals(object? obj) => obj is Bitboard bb && this == bb;

        // 等価演算子をオーバーロードした場合, GetHashCodeメソッドをオーバーライドしないと警告が出る.
        public override readonly int GetHashCode() => base.GetHashCode();

        public readonly ulong CalculatePlayerMobility() => CalculateMobility(Player, Opponent);
        public readonly ulong CalculateOpponentMobility() => CalculateMobility(Opponent, Player);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly ulong CalculateFlippedDiscs(BoardCoordinate coord)
        {
            if (Avx2.X64.IsSupported)
                return CalculateFilippedDiscs_AVX2(Player, Opponent, (byte)coord);
            else if (Sse2.IsSupported)
                return CalculateFlippedDiscs_SSE(Player, Opponent, (byte)coord);
            return CalculateFlippedDiscs_CPU(Player, Opponent, (byte)coord);
        }

        public void PutPlayerDiscAt(BoardCoordinate coord)
        {
            var bit = CoordToBit[(byte)coord];
            Player |= bit;
        ...
        {
            var player = Player;
            Player = Opponent ^= flipped;
            Opponent = player | (CoordToBit[(byte)coord] | flipped);
        }

        public void Undo(BoardCoordinate coord, ulong flipped)
        {
            var player = Player;
            Player = Opponent ^ (CoordToBit[(byte)coord] | flipped);
            Opponent = player | flipped;
        }

        public void Swap() => (Player, Opponent) = (Opponent, Player);
        ...
        {
            var coord_bit = CoordToBit[coord];
            var masked_o = o & 0x7e7e7e7e7e7e7e7eUL;
